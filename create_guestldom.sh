...
## PrintSummary
##
## Print summary report at script end
##
## usage: called by the runitme system
##
## returns:  n/a
##          
##
function PrintSummary {
  typeset __FUNCTION="PrintSummary";   ${__FUNCTION_INIT} ;  
  ${__DEBUG_CODE}

  if [ "${PREFIX}"x = ""x ] ; then
    LogMsg "# Summary:"
    LogMsg "#   Guest LDoms successfully created: "
    LogMsg "#    ${GUEST_LDOMS_CREATED}"
    LogMsg "#   Guest LDoms not successfully created: "
    LogMsg "#     ${GUEST_LDOMS_WITH_ERRORS}"
  fi
  
  if [ "${OUTPUT_SCRIPT}"x != ""x ] ; then
    LogMsg "All commands were written to \"${OUTPUT_SCRIPT}\" and NOT executed."
  elif [ "${PREFIX}"x != ""x  ] ; then
    LogMsg "All commands were only printed and NOT executed."
  fi
...
##
## usage: DoCommand "command_to_execute
##
## returns:  return code of the command (or 0 for printing)
##          
##
function DoCommand {
  typeset __FUNCTION="DoCommand";   ${__FUNCTION_INIT} ;  
  ${__DEBUG_CODE}

  typeset THIS_COMMAND="$*"
  
  if [ "${OUTPUT_SCRIPT}"x != ""x ] ; then
    echo "${THIS_COMMAND}" >>"${OUTPUT_SCRIPT}"
    THISRC=$?
  else
    if [ "${PREFIX}"x != ""x ] ; then
      ${PREFIX} "${THIS_COMMAND}"
      ...
      LogInfo "Executing \"${THIS_COMMAND}\" ...."
      ${THIS_COMMAND}
      THISRC=$?
    fi
...
#
  ...
  __GETOPTS="+:ynvqhHDfl:aOS:CVTxd:s:biL"
  if [ "${__OS}"x = "SunOS"x  ] ; then
    if [ "${__OS_VERSION}"x  = "5.10"x -o  "${__OS_VERSION}"x  = "5.11"x ] ; then
      __GETOPTS="+:y(yes)n(no)v(verbose)q(quiet)h(help)H(doc)D(debug)f(force)l:(logfile)a(color)O(overwrite)S:(summaries)C(writeconfigfile)V(version)T(tee)x(doit)d:(guestldom)s:(script)b(bindldom)i(createimagefile)L(list)"
    fi
  fi
      ...
      "+n" ) __USER_RESPONSE_IS="" ;;
       ...
       "S" ) case ${OPTARG} in
        ...
        "+x" ) PREFIX="echo"
         ...
         "i" ) CREATE_IMAGE_FILE=${__TRUE}
                ;;

        "+i" ) CREATE_IMAGE_FILE=${__FALSE}
                ;;
                
         "b" ) BIND_LDOMS=${__TRUE}
                ;;

        "+b" ) BIND_LDOMS=${__FALSE}
                ;;

         "L" ) LIST_LDOMS=${__TRUE}
                ;;

        "+L" ) LIST_LDOMS=${__FALSE}
                ;;

         "d" ) GUEST_LDOMS="${GUEST_LDOMS} ${OPTARG}"
                 ;;

        "+d" ) GUEST_LDOMS="${OPTARG}"
                 ;;

         "s" ) OUTPUT_SCRIPT="${OPTARG}"
                ;;
...
# check Guest LDom definitions
#
  if [ "${NO_OF_DEFINED_GUEST_LDOMS}"x = ""x -o "${NO_OF_DEFINED_GUEST_LDOMS}"x = "0"x ] ; then
    die 70 "No Guest LDoms defined; ${SCRIPT_USER_MSG}: Did you read the documentation and created the neccessary infrastructure for this script?"
  fi

# create the list of defined Guest LDoms
#   
    LogInfo "Creating the list of defined Guest LDoms ..."
    i=0
    LIST_OF_DEFINED_GUEST_LDOMS=""
    while [ $i -le ${NO_OF_DEFINED_GUEST_LDOMS} ] ;  do
      (( i = i + 1 ))
      LIST_OF_DEFINED_GUEST_LDOMS="${LIST_OF_DEFINED_GUEST_LDOMS} ${GUEST_LDOM_NAME[$i]}"
    done

# process parameter -L and exit 
#
  if [ "${LIST_LDOMS}" = ${__TRUE} ] ; then
    LogInfo "Parameter -L found -- all other parameter will be ignored"
    LogMsg "Defined Guest LDoms:"
    for CURLDOM in ${LIST_OF_DEFINED_GUEST_LDOMS} ; do
      if [ ${__NO_HEADERS} = ${__TRUE} ] ; then
        echo ${CURLDOM}
      else
        LogMsg "-" ${CURLDOM}
      fi
...
# check requested Guest LDoms
#
  LDOM_INDEX_LIST=""
  ALL_LDOMS_DEFINED=${__TRUE}

  pos " all " "${GUEST_LDOMS} "
  [ $? -ne 0 ] && GUEST_LDOMS="${GUEST_LDOMS} ${LIST_OF_DEFINED_GUEST_LDOMS}"
    
  LogInfo "Checking the definitions of the requested Guest LDoms ..."
  for CUR_LDOM in ${GUEST_LDOMS} ; do
    [ "${CUR_LDOM}"x = "all"x ] && continue

    LogInfo "  Checking the guest LDom definition for \"${CUR_LDOM}\" ..."
    i=0 
    CUR_LDOM_DEFINED=${__FALSE}
    while [ $i -le ${NO_OF_DEFINED_GUEST_LDOMS} ] ;  do
      (( i = i + 1 ))
      if [ "${GUEST_LDOM_NAME[$i]}"x = "${CUR_LDOM}"x ] ; then
        pos $i ${LDOM_INDEX_LIST}
        [ $? -eq 0 ] && LDOM_INDEX_LIST=" ${LDOM_INDEX_LIST} ${i} "
        CUR_LDOM_DEFINED=${__TRUE}
        break
      fi
    done

    if [ ${CUR_LDOM_DEFINED} !=  ${__TRUE} ] ; then
      LogError "Guest LDom \"${CUR_LDOM}\" is NOT defined"
      ALL_LDOMS_DEFINED=${__FALSE}
    else
      LogInfo "  OK: Guest LDom \"${CUR_LDOM}\" is defined"
    fi

  done

  [ ${ALL_LDOMS_DEFINED} = ${__FALSE} ] && die 20 "Not all requested Guest LDoms defined, please try again ${SCRIPT_USER_MSG}"

  if [ "${PREFIX}"x = ""x -a "${OUTPUT_SCRIPT}"x = ""x ] ; then
    [ ! -x "${LDM}" ] && die 25 "The binary \"${LDM}\" does not exist or is not executable"
   UserIsRoot || die 249 "You must be root to execute this script in execute mode" 
 fi

  if [ "${OUTPUT_SCRIPT}"x != ""x ] ; then
    LogMsg "All commands are written to the file \"${OUTPUT_SCRIPT}\" "
    cat >>"${OUTPUT_SCRIPT}" <<EOT
#
# ${OUTPUT_SCRIPT} created by ${__SCRIPTNAME} at $( date )
#
EOT
    [ $? -ne 0 ] && die 50 "Can not write to the file \"${OUTPUT_SCRIPT}\" "
  fi

  GUEST_LDOMS_CREATED=""
  GUEST_LDOMS_WITH_ERRORS=""

  __EXITROUTINES="${__EXITROUTINES} PrintSummary"    

  for CUR_LDOM_INDEX in ${LDOM_INDEX_LIST} ; do
    CMD_EXECUTION_ERRORS=${__FALSE}  

    export GUEST_LDOM_NAME="${GUEST_LDOM_NAME[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_HOSTID="${GUEST_LDOM_HOSTID[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_MACADDRESS="${GUEST_LDOM_MACADDRESS[${CUR_LDOM_INDEX}]}"
  
    export GUEST_LDOM_VCONSOLE_PORT="${GUEST_LDOM_VCONSOLE_PORT[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_VCONSOLE_GROUP="${GUEST_LDOM_VCONSOLE_GROUP[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_VCONSOLE_SERVICE="${GUEST_LDOM_VCONSOLE_SERVICE[${CUR_LDOM_INDEX}]}"
    
    export GUEST_LDOM_OBP_VARIABLES="${GUEST_LDOM_OBP_VARIABLES[${CUR_LDOM_INDEX}]}"
  
    export GUEST_LDOM_CORES="${GUEST_LDOM_CORES[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_MAU="${GUEST_LDOM_MAU[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_MEMORY="${GUEST_LDOM_MEMORY[${CUR_LDOM_INDEX}]}"
 
    export GUEST_LDOM_IMAGEFILE="${GUEST_LDOM_IMAGEFILE[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_IMAGEFILE_OPTIONS="${GUEST_LDOM_IMAGEFILE_OPTIONS[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_IMAGEFILE_VDISKSERVER_OPTIONS="${GUEST_LDOM_IMAGEFILE_VDISKSERVER_OPTIONS[$i]}"
  
    export GUEST_LDOM_NETWORK_ADAPTER0_SWITCH="${GUEST_LDOM_NETWORK_ADAPTER0_SWITCH[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_NETWORK_ADAPTER0_MAC_ADDRESS="${GUEST_LDOM_NETWORK_ADAPTER0_MAC_ADDRESS[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_NETWORK_ADAPTER0_OPTIONS="${GUEST_LDOM_NETWORK_ADAPTER0_OPTIONS[${CUR_LDOM_INDEX}]}"
    
    export GUEST_LDOM_NETWORK_ADAPTER1_SWITCH="${GUEST_LDOM_NETWORK_ADAPTER1_SWITCH[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_NETWORK_ADAPTER1_MAC_ADDRESS="${GUEST_LDOM_NETWORK_ADAPTER1_MAC_ADDRESS[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_NETWORK_ADAPTER1_OPTIONS="${GUEST_LDOM_NETWORK_ADAPTER1_OPTIONS[${CUR_LDOM_INDEX}]}"

    export GUEST_LDOM_NETWORK_ADAPTER2_SWITCH="${GUEST_LDOM_NETWORK_ADAPTER2_SWITCH[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_NETWORK_ADAPTER2_MAC_ADDRESS="${GUEST_LDOM_NETWORK_ADAPTER2_MAC_ADDRESS[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_NETWORK_ADAPTER2_OPTIONS="${GUEST_LDOM_NETWORK_ADAPTER2_OPTIONS[${CUR_LDOM_INDEX}]}"

    export GUEST_LDOM_NETWORK_ADAPTER3_SWITCH="${GUEST_LDOM_NETWORK_ADAPTER3_SWITCH[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_NETWORK_ADAPTER3_MAC_ADDRESS="${GUEST_LDOM_NETWORK_ADAPTER3_MAC_ADDRESS[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_NETWORK_ADAPTER3_OPTIONS="${GUEST_LDOM_NETWORK_ADAPTER3_OPTIONS[${CUR_LDOM_INDEX}]}"

    export GUEST_LDOM_NETWORK_ADAPTER4_SWITCH="${GUEST_LDOM_NETWORK_ADAPTER4_SWITCH[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_NETWORK_ADAPTER4_MAC_ADDRESS="${GUEST_LDOM_NETWORK_ADAPTER4_MAC_ADDRESS[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_NETWORK_ADAPTER4_OPTIONS="${GUEST_LDOM_NETWORK_ADAPTER4_OPTIONS[${CUR_LDOM_INDEX}]}"

    export GUEST_LDOM_NETWORK_ADAPTER5_SWITCH="${GUEST_LDOM_NETWORK_ADAPTER5_SWITCH[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_NETWORK_ADAPTER5_MAC_ADDRESS="${GUEST_LDOM_NETWORK_ADAPTER5_MAC_ADDRESS[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_NETWORK_ADAPTER5_OPTIONS="${GUEST_LDOM_NETWORK_ADAPTER7_OPTIONS[${CUR_LDOM_INDEX}]}"

    export GUEST_LDOM_NETWORK_ADAPTER6_SWITCH="${GUEST_LDOM_NETWORK_ADAPTER6_SWITCH[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_NETWORK_ADAPTER6_MAC_ADDRESS="${GUEST_LDOM_NETWORK_ADAPTER6_MAC_ADDRESS[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_NETWORK_ADAPTER6_OPTIONS="${GUEST_LDOM_NETWORK_ADAPTER6_OPTIONS[${CUR_LDOM_INDEX}]}"
  
    export GUEST_LDOM_NETWORK_ADAPTER7_SWITCH="${GUEST_LDOM_NETWORK_ADAPTER7_SWITCH[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_NETWORK_ADAPTER7_MAC_ADDRESS="${GUEST_LDOM_NETWORK_ADAPTER7_MAC_ADDRESS[${CUR_LDOM_INDEX}]}"
    export GUEST_LDOM_NETWORK_ADAPTER7_OPTIONS="${GUEST_LDOM_NETWORK_ADAPTER7_OPTIONS[${CUR_LDOM_INDEX}]}"

    export GUEST_LDOM_PREINSTALL_SCRIPT="${GUEST_LDOM_PREINSTALL_SCRIPT[$i]}"
    export GUEST_LDOM_POSTINSTALL_SCRIPT="${GUEST_LDOM_POSTINSTALL_SCRIPT[$i]}"

    (( GUEST_LDOM_THREADS = ${GUEST_LDOM_CORES} * ${THREADS_PER_CORE} ))

    export GUEST_LDOM_CREATED=${__FALSE}    
    COMMANDS_WITH_ERRORS=""

    if [ "${PREFIX}"x  = ""x -a "${OUTPUT_SCRIPT}"x = ""x ] ; then
      LogInfo "Checking if the Guest LDom is already configured ..."
      $LDM list ${GUEST_LDOM_NAME} 2>/dev/null
      if [ $? -eq 0 ] ; then
        LogError "The Guest LDom \"${GUEST_LDOM_NAME}\" already exists; doing nothing now."
        continue
      fi
      DoCommand "echo" "# Creating the Guest LDom \"${GUEST_LDOM_NAME}\" ..."
    else
      DoCommand "echo" "# Commands to create the Guest LDom \"${GUEST_LDOM_NAME}\": "
    fi
    
    if [ "${GUEST_LDOM_PREINSTALL_SCRIPT}"x != ""x ] ; then
      LogInfo "Preinstall script for the the Guest LDom defined"
      if [ ! -x "${GUEST_LDOM_PREINSTALL_SCRIPT}" ] ; then
        LogError "Preinstall script \"${GUEST_LDOM_PREINSTALL_SCRIPT}\" not found or not executable; Guest LDom NOT configured"
        GUEST_LDOMS_WITH_ERRORS="${GUEST_LDOMS_WITH_ERRORS} ${GUEST_LDOM_NAME}"
        continue
      fi

      LogMsg "Processing \"${GUEST_LDOM_PREINSTALL_SCRIPT}\" ..."
      DoCommand   "${GUEST_LDOM_PREINSTALL_SCRIPT}"
      if [ $? -ne 0 ] ; then
        LogError "Preinstallscript \"${GUEST_LDOM_PREINSTALL_SCRIPT}\" ended with RC not equal zero; Guest LDom NOT configured"
        GUEST_LDOMS_WITH_ERRORS="${GUEST_LDOMS_WITH_ERRORS} ${GUEST_LDOM_NAME}"
        continue
      fi
    fi

    if [ "$( echo ${GUEST_LDOM_IMAGEFILE} | cut -c1 )"x = "/"x  ] ; then
      export GUEST_LDOM_DISKDIR="$( basedir ${GUEST_LDOM_IMAGEFILE} )"
    else
      export GUEST_LDOM_DISKDIR="${DISKDIR}/${GUEST_LDOM_NAME}/rootdisk"
    fi
    
    if [ ${CREATE_IMAGE_FILE} = ${__TRUE} ] ; then
      if [ ! -d "${GUEST_LDOM_DISKDIR}" ] ; then
        DoCommand mkdir -p "${GUEST_LDOM_DISKDIR}" 
        [ $? -ne 0 ] && LogError "Error creating the directory \"${GUEST_LDOM_DISKDIR}\" for the root disk file image"
      else
        PREFIX="echo"  DoCommand "# mkdir -p ${GUEST_LDOM_DISKDIR}" "# Info only: directory already exists "
      fi
      
      if  [ -f "${GUEST_LDOM_DISKDIR}/${GUEST_LDOM_IMAGEFILE}" ]  ; then
        PREFIX="echo"  DoCommand  "# mkfile -n ${BOOT_DISK_IMG_SIZE} \"${GUEST_LDOM_DISKDIR}/${GUEST_LDOM_IMAGEFILE}\" " "# Info only: file already exists "
        else
        DoCommand  mkfile -n ${BOOT_DISK_IMG_SIZE} "${GUEST_LDOM_DISKDIR}/${GUEST_LDOM_IMAGEFILE}"
      fi
    fi

    DoCommand "echo" "# Configuring the Guest LDom \"${GUEST_LDOM_NAME}\" ..."

    LDOM_VDISK_SERVER_DEVICE=""
    VDISK_SERVER_DEV="$( ${LDM} list -l -p ${SERVICE_DOMAIN} | grep ${GUEST_LDOM_DISKDIR}/${GUEST_LDOM_IMAGEFILE}  | cut -f2 -d "|" | cut -f2 -d "=" )"
    if [ "${VDISK_SERVER_DEV}"x != ""x ] ; then
      LDOM_VDISK_SERVER_DEVICE="${VDISK_SERVER_DEV}@${VDISK_SERVER_SWITCH}"
      LogInfo "The vdisk server device is already defined: \"${LDOM_VDISK_SERVER_DEVICE}\" "    
      PREFIX="echo" DoCommand "# ${LDM} add-vdiskserverdevice ${GUEST_LDOM_IMAGEFILE_VDISKSERVER_OPTIONS} ${GUEST_LDOM_DISKDIR}/${GUEST_LDOM_IMAGEFILE} ${VDISK_SERVER_DEV}"  "# Info only: vdisk server device is already configured "
    else
      VDISK_SERVER_DEV="${GUEST_LDOM_NAME}_bootdisk@${VDISK_SERVER_SWITCH}"
      DoCommand "${LDM} add-vdiskserverdevice ${GUEST_LDOM_IMAGEFILE_VDISKSERVER_OPTIONS} ${GUEST_LDOM_DISKDIR}/${GUEST_LDOM_IMAGEFILE} ${VDISK_SERVER_DEV}"  
      [ $? -eq 0 ] && LDOM_VDISK_SERVER_DEVICE="${VDISK_SERVER_DEV}"
    fi

    LDM_COMMAND="${LDM} add-domain"
    [ "${GUEST_LDOM_HOSTID}"x != ""x ] && LDM_COMMAND="${LDM_COMMAND} hostid=${GUEST_LDOM_HOSTID} "
    [ "${GUEST_LDOM_MACADDRESS}"x != ""x ] && LDM_COMMAND="${LDM_COMMAND} mac-addr=${GUEST_LDOM_MACADDRESS} "
    LDM_COMMAND="${LDM_COMMAND} ${GUEST_LDOM_NAME}"
    DoCommand "${LDM_COMMAND}" || continue
  
    [ "${GUEST_LDOM_THREADS}"x != ""x ] && \
      DoCommand  "${LDM} add-vcpu ${GUEST_LDOM_THREADS} ${GUEST_LDOM_NAME}" 

    [ "${GUEST_LDOM_MAU}"x != ""x -a "${GUEST_LDOM_MAU}"x != "0"x ] && \
      DoCommand  "${LDM} add-crypto ${GUEST_LDOM_MAU} ${GUEST_LDOM_NAME}"

    [ "${GUEST_LDOM_MEMORY}"x != ""x  ] && \
      DoCommand  "${LDM} add-memory ${GUEST_LDOM_MEMORY} ${GUEST_LDOM_NAME}"

    [ "${GUEST_LDOM_IMAGEFILE}"x != ""x  -a "${LDOM_VDISK_SERVER_DEVICE}"x != ""x ] && \
      DoCommand "${LDM} add-vdisk vdisk0 ${GUEST_LDOM_IMAGEFILE_OPTIONS} ${LDOM_VDISK_SERVER_DEVICE} ${GUEST_LDOM_NAME}" 
    
    if [ "${GUEST_LDOM_NETWORK_ADAPTER0_SWITCH}"x != ""x ] ; then
      LDM_COMMAND="${LDM} add-vnet ${GUEST_LDOM_NETWORK_ADAPTER0_OPTIONS}"
      [ "${GUEST_LDOM_NETWORK_ADAPTER0_MAC_ADDRESS}"x != ""x ] && \
        LDM_COMMAND="${LDM_COMMAND} mac-addr=${GUEST_LDOM_NETWORK_ADAPTER0_MAC_ADDRESS} "
      LDM_COMMAND="${LDM_COMMAND} vnet0 ${GUEST_LDOM_NETWORK_ADAPTER0_SWITCH} ${GUEST_LDOM_NAME}"
      DoCommand  "${LDM_COMMAND}" 
    fi

    if [ "${GUEST_LDOM_NETWORK_ADAPTER1_SWITCH}"x != ""x ] ; then
      LDM_COMMAND="${LDM} add-vnet ${GUEST_LDOM_NETWORK_ADAPTER1_OPTIONS}"
      [ "${GUEST_LDOM_NETWORK_ADAPTER1_MAC_ADDRESS}"x != ""x ] && \
        LDM_COMMAND="${LDM_COMMAND} mac-addr=${GUEST_LDOM_NETWORK_ADAPTER1_MAC_ADDRESS} "
      LDM_COMMAND="${LDM_COMMAND} vnet1 ${GUEST_LDOM_NETWORK_ADAPTER1_SWITCH} ${GUEST_LDOM_NAME}"
      DoCommand  "${LDM_COMMAND}" 
    fi

    if [ "${GUEST_LDOM_NETWORK_ADAPTER2_SWITCH}"x != ""x ] ; then
      LDM_COMMAND="${LDM} add-vnet ${GUEST_LDOM_NETWORK_ADAPTER2_OPTIONS}"
      [ "${GUEST_LDOM_NETWORK_ADAPTER2_MAC_ADDRESS}"x != ""x ] && \
        LDM_COMMAND="${LDM_COMMAND} mac-addr=${GUEST_LDOM_NETWORK_ADAPTER2_MAC_ADDRESS} "
      LDM_COMMAND="${LDM_COMMAND} vnet2 ${GUEST_LDOM_NETWORK_ADAPTER2_SWITCH} ${GUEST_LDOM_NAME}"
      DoCommand  "${LDM_COMMAND}" 
    fi

    if [ "${GUEST_LDOM_NETWORK_ADAPTER3_SWITCH}"x != ""x ] ; then
      LDM_COMMAND="${LDM} add-vnet ${GUEST_LDOM_NETWORK_ADAPTER3_OPTIONS}"
      [ "${GUEST_LDOM_NETWORK_ADAPTER3_MAC_ADDRESS}"x != ""x ] && \
        LDM_COMMAND="${LDM_COMMAND} mac-addr=${GUEST_LDOM_NETWORK_ADAPTER3_MAC_ADDRESS} "
      LDM_COMMAND="${LDM_COMMAND} vnet3 ${GUEST_LDOM_NETWORK_ADAPTER3_SWITCH} ${GUEST_LDOM_NAME}"
      DoCommand  "${LDM_COMMAND}" 
    fi

    if [ "${GUEST_LDOM_NETWORK_ADAPTER4_SWITCH}"x != ""x ] ; then
      LDM_COMMAND="${LDM} add-vnet ${GUEST_LDOM_NETWORK_ADAPTER4_OPTIONS}"
      [ "${GUEST_LDOM_NETWORK_ADAPTER4_MAC_ADDRESS}"x != ""x ] && \
        LDM_COMMAND="${LDM_COMMAND} mac-addr=${GUEST_LDOM_NETWORK_ADAPTER4_MAC_ADDRESS} "
      LDM_COMMAND="${LDM_COMMAND} vnet4 ${GUEST_LDOM_NETWORK_ADAPTER4_SWITCH} ${GUEST_LDOM_NAME}"
      DoCommand  "${LDM_COMMAND}" 
    fi

    if [ "${GUEST_LDOM_NETWORK_ADAPTER5_SWITCH}"x != ""x ] ; then
      LDM_COMMAND="${LDM} add-vnet ${GUEST_LDOM_NETWORK_ADAPTER5_OPTIONS}"
      [ "${GUEST_LDOM_NETWORK_ADAPTER5_MAC_ADDRESS}"x != ""x ] && \
        LDM_COMMAND="${LDM_COMMAND} mac-addr=${GUEST_LDOM_NETWORK_ADAPTER5_MAC_ADDRESS} "
      LDM_COMMAND="${LDM_COMMAND} vnet5 ${GUEST_LDOM_NETWORK_ADAPTER5_SWITCH} ${GUEST_LDOM_NAME}"
      DoCommand  "${LDM_COMMAND}" 
    fi

    if [ "${GUEST_LDOM_NETWORK_ADAPTER6_SWITCH}"x != ""x ] ; then
      LDM_COMMAND="${LDM} add-vnet ${GUEST_LDOM_NETWORK_ADAPTER6_OPTIONS}"
      [ "${GUEST_LDOM_NETWORK_ADAPTER6_MAC_ADDRESS}"x != ""x ] && \
        LDM_COMMAND="${LDM_COMMAND} mac-addr=${GUEST_LDOM_NETWORK_ADAPTER6_MAC_ADDRESS} "
      LDM_COMMAND="${LDM_COMMAND} vnet6 ${GUEST_LDOM_NETWORK_ADAPTER6_SWITCH} ${GUEST_LDOM_NAME}"
      DoCommand  "${LDM_COMMAND}" 
    fi

    if [ "${GUEST_LDOM_NETWORK_ADAPTER7_SWITCH}"x != ""x ] ; then
      LDM_COMMAND="${LDM} add-vnet ${GUEST_LDOM_NETWORK_ADAPTER7_OPTIONS}"
      [ "${GUEST_LDOM_NETWORK_ADAPTER7_MAC_ADDRESS}"x != ""x ] && \
        LDM_COMMAND="${LDM_COMMAND} mac-addr=${GUEST_LDOM_NETWORK_ADAPTER7_MAC_ADDRESS} "
      LDM_COMMAND="${LDM_COMMAND} vnet7 ${GUEST_LDOM_NETWORK_ADAPTER7_SWITCH} ${GUEST_LDOM_NAME}"
      DoCommand  "${LDM_COMMAND}" 
    fi

    for CUR_OBP_VAR in ${GUEST_LDOM_OBP_VARIABLES} ; do
      LDM_COMMAND="${LDM} set-variable ${CUR_OBP_VAR}  ${GUEST_LDOM_NAME}"
      DoCommand  "${LDM_COMMAND}" 
    done

    if [ "${GUEST_LDOM_VCONSOLE_PORT}"x != ""x ] ; then
      LDM_COMMAND="${LDM} set-console port=${GUEST_LDOM_VCONSOLE_PORT}"   
      [ "${GUEST_LDOM_VCONSOLE_GROUP}"x != ""x ] && LDM_COMMAND="${LDM_COMMAND} group=${GUEST_LDOM_VCONSOLE_GROUP}"
      [ "${GUEST_LDOM_VCONSOLE_SERVICE}"x != ""x ] && LDM_COMMAND="${LDM_COMMAND} service=${GUEST_LDOM_VCONSOLE_SERVICE}"
      LDM_COMMAND="${LDM_COMMAND} ${GUEST_LDOM_NAME}"
      DoCommand  "${LDM_COMMAND}" 
    fi

    if [ ${CMD_EXECUTION_ERRORS} = ${__TRUE} ] ; then
      LogError "Commands with errors for this Guest LDom:"
      LogMsg "-" "${COMMANDS_WITH_ERRORS}"
      LogError "${SCRIPT_USER_MSG}, please check your Guest LDom config file - maybe it's time to read the documentaton again?"
      GUEST_LDOMS_WITH_ERRORS="${GUEST_LDOMS_WITH_ERRORS} ${GUEST_LDOM_NAME}"
      continue
    fi
   
    if [ ${BIND_LDOMS} = ${__TRUE} ] ; then
# Bind the Guest LDoms      
      DoCommand ${LDM} bind-domain "${GUEST_LDOM_NAME}"
      THISRC=$?
      if [ ${THISRC} -ne 0 ] ; then
        LogError "Error ${THISRC} binding the Guest Lom \"${GUEST_LDOM_NAME}\" "
        GUEST_LDOMS_WITH_ERRORS="${GUEST_LDOMS_WITH_ERRORS} ${GUEST_LDOM_NAME}"
      else
        GUEST_LDOMS_CREATED="${GUEST_LDOMS_CREATED} ${GUEST_LDOM_NAME}"
      fi
    else
      LogInfo "Guest LDom \"${GUEST_LDOM_NAME}\" not bound"
      GUEST_LDOMS_CREATED="${GUEST_LDOMS_CREATED} ${GUEST_LDOM_NAME}"
    fi

    if [ "${GUEST_LDOM_POSTINSTALL_SCRIPT}"x != ""x ] ; then
      LogInfo "Postinstall script for the the guest LDom defined"
      if [ ! -x "${GUEST_LDOM_POSTINSTALL_SCRIPT}" ] ; then
        LogError "Preinstallscript \"${GUEST_LDOM_POSTINSTALL_SCRIPT}\" not found or not executable"
        GUEST_LDOMS_WITH_ERRORS="${GUEST_LDOMS_WITH_ERRORS} ${GUEST_LDOM_NAME}"
        continue
      fi
      LogMsg "Processing \"${GUEST_LDOM_POSTINSTALL_SCRIPT}\" ..."
      DoCommand "${GUEST_LDOM_POSTINSTALL_SCRIPT}"
      if [ $? -ne 0 ] ; then
        LogError "Postinstallscript \"${GUEST_LDOM_PREINSTALL_SCRIPT}\" ended with RC not equal zero"
        continue
      fi
    fi
