...
# Notes:
#
# - use "execute_on_all_hosts.sh {-v} {-v} -h" to get the usage help
#
...
##                         Tested with Solaris, Linux, and CYGWIN
##
##
## Description
## 
...
##
##       http://home.arcor.de/bnsmb/public/htdocs/scriptt.html
##
##
## Credits
...
##     corrected a nasty syntax error 
...
##
typeset  __SCRIPT_VERSION="v0.0.7"
##
...
# this variable is set to true after the processing started 
# it is used in the routine mycleanup
#
PROCESSING_STARTED=${__FALSE}
...
# default for the parameter -I
  DEFAULT_FILE_BASEDIR="/cygdrive/c/data/hostlists"
  
# default for the parameter -s  
  DEFAULT_SCRIPTFILE=""

# default for the parameter -o  
  DEFAULT_OUTPUTFILE="/var/tmp/${__SCRIPTNAME##*/}.$$.log"
...
# variables that can NOT be changed via parameter

# binaries used
  DEFAULT_SSH_BINARY="ssh"
  DEFAULT_SCP_BINARY="scp"
  DEFAULT_DOS2UNIX="$( whence dos2unix )"

# options for ssh -- default for the parameter -P
  DEFAULT_SSH_OPTIONS=""
  
# options for scp -- default for the parameter -p
  DEFAULT_SCP_OPTIONS=""
...
## Note: CYGWIN ksh does not like typeset -r
##
typeset  __SHORT_DESC="copy a script to a list of hosts via scp and execute it on the hosts using ssh"
...
__LONG_USAGE_HELP='
      -i hostlist
              hostlist is the file with the list of hosts to process; add one hostid per line
      ...
      -s scriptfile
              scriptfile is the script to execute on all hosts; it will be executed on the hosts
              with the shell \"${SHELL_TO_USE}\"
              Long format: --scriptfile

      -I basedir
              directory with the hostlist and the script files; if this parameter 
              is found the script searches the hostlist files and the script files
              in this directory also
              Long format: --basedir
              Current value: \"${FILE_BASEDIR}\"

      -o outputfile
              outputfile is the name of the file for the output generated by the commands
              Current value \"${OUTPUTFILE}\"; the parameter is optional
              Long format: --outputfile

      -u sshuser
              sshuser is the userid to use for ssh and scp on the target host
              Current value \"${SSHUSER}\"; the parameter is optional
              Long format: --sshuser

      -p scpoptions
              additional options for scp, e.g 
      ...
      -c shell_to_use
              ...
              Current value \"${SHELL_TO_USE}\"; the parameter is optional
              Long format: --shell

      -k|+k do add comments to the output file
              Current value: $( ConvertToYesNo "${ADD_COMMENTS}" )
              Long format: --nocomments

      -K|+k use the ssh option StrictHostKeyChecking=no for ssh and scp
...
##
__SHORT_USAGE_HELP='                    -i hostlist{,hostlist1{,...} -s scriptfile [-o outputfile] [-I basedir]
                    [-u userid] [-c shell] [-k] [-U] [-p scpoptions] [-P sshoptions] [-K]
                    [hostlist [scriptfile [outputfile [sshuser]]]]
  The parameters overwrite the options; hostlist and scriptfile are mandatory either
...
function executeCommandAndLog {
  ...
  if [ "${__LOGFILE}"x != ""x -a -f "${__LOGFILE}" ] ; then
    typeset CMDOUTPUT=$( eval "$@"  2>&1 )
    THISRC=$?
#    eval "$@"  2>&1 | tee -a "${__LOGFILE}"    
     echo "${CMDOUTPUT}" 2>&1 | tee -a "${__LOGFILE}"    
  else    
    eval "$@"
    THISRC=$?
  fi
...
cat <<EOT
 Note: Use -{switch} or --{longswitch} to turn an option on; 
       ...
       The long format of the parameter (--parameter/++parameter) is not supported by 
       all ksh implementations
...
#
  ...
  __GETOPTS="+:ynvqhHDfl:aOS:CVTi:o:s:u:c:kUI:p:P:K"
  if [ "${__OS}"x = "SunOS"x  ] ; then
    if [ "${__OS_VERSION}"x  = "5.10"x -o  "${__OS_VERSION}"x  = "5.11"x ] ; then
      __GETOPTS="+:y(yes)n(no)v(verbose)q(quiet)h(help)H(doc)D(debug)f(force)l:(logfile)a(color)O(overwrite)S:(summaries)C(writeconfigfile)V(version)T(tee)i:(hostlist)o:(outputfile)s:(scriptfile)u:(sshuser)c:(shell)k(nocomments)U(uniquelogfiles)I:(basedir)p:(scpoptions)P:(sshoptions)K(nostrictkeys)"
    fi
  fi
...
# "
  ...
  if [ "${HOSTFILE}"x = ""x ] ; then
    PARAMETER_OKAY=${__FALSE}
    LogError "hostlist not specified"
  else
    for CURFILE in $( echo ${HOSTFILE} | tr -s "," " " ) ; do
      if [ ! -r "${CURFILE}" ] ; then
        if [ "${FILE_BASEDIR}" != ""x -a ! -r "${FILE_BASEDIR}/${CURFILE}" ] ; then
          LogError "Can not read the file \"${CURFILE}\" "
          PARAMETER_OKAY=${__FALSE}
        else
          HOSTFILE_LIST="${HOSTFILE_LIST} ${FILE_BASEDIR}/${CURFILE}"
        fi
      else
        HOSTFILE_LIST="${HOSTFILE_LIST} ${CURFILE}"
      fi
    done
  fi
  ...
  if [ "${SCRIPTFILE}"x = ""x ] ; then
    PARAMETER_OKAY=${__FALSE}
    LogError "scriptfile not specified"
  ...
  if [[ ${__OS} == CYGWIN*  ]] ; then
    if [ "${DOS2UNIX}"x != ""x ] ; then
      LogMsg "Calling dos2unix for the script file \"${SCRIPTFILE}\" "
      ${DOS2UNIX} "${SCRIPTFILE}"
    else
      LogInfo "dos2unix not found in the PATH - can not convert the script file"
    fi
  fi
  ...
  if [ "${SSHUSER}"x = ""x ] ; then
    PARAMETER_OKAY=${__FALSE}
    LogError "sshuser not specified and no default value found"
  fi
  ...
  TARGET_TMPFILE="/tmp/tmp_$$_$( basename $0 )"
  LogMsg ""
  LogMsg "Executing the script "
  LogMsg "    ${SCRIPTFILE}"
  LogMsg "as ssh user "
  LogMsg "    ${SSHUSER}"
  ...
  LogMsg ""
  LogMsg "The shell to execute the script is "
  LogMsg "    ${SHELL_TO_USE}"
  LogMsg ""
...
# check SSH agent
  ...
  LogMsg ""
  LogInfo "The temporary file on the hosts is \"${TARGET_TMPFILE}\"" 
  
  
  TEMP_LIST=""
  for CUR_HOSTFILE in ${HOSTFILE_LIST} ; do
    TEMP_LIST="${TEMP_LIST} $( cat "${CUR_HOSTFILE}" | tr -d "\r" | tr -s " " | grep -v "^#" | grep -v "^$" ) "
  done
  LIST_OF_HOSTS_TO_PROCESS="$( echo "${TEMP_LIST}" | sort | uniq )"
  NO_OF_HOSTS="$( echo "${LIST_OF_HOSTS_TO_PROCESS}"  | wc -l )"
  ...
  for CURMACHINE in ${LIST_OF_HOSTS_TO_PROCESS} ; do
      (( COUNT = COUNT + 1 ))
      LogMsg  "---- Processing \"${CURMACHINE}\" ... ( ${COUNT} from ${NO_OF_HOSTS}) "
    
      if [ ${UNIQUE_LOGFILES} = ${__FALSE} ] ; then
        CUR_OUTPUTFILE="${OUTPUTFILE}"
      else
        CUR_OUTPUTFILE="${OUTPUTFILE}.${CURMACHINE}"
        if [ ${__OVERWRITE_MODE} = ${__TRUE} ] ; then
        ...
        if [ ${THISRC} -ne 0 ] ; then
          LogError "Can not write to the logfile \"${CUR_OUTPUTFILE}\" -- skipping the host \"${CURMACHINE}\" "
          INVALID_HOSTS_LIST="${INVALID_HOSTS_LIST} ${CURMACHINE}"
          continue
        fi
      fi

      executeCommand "${SCP_BINARY}" "${SCP_OPTIONS}" "${SCRIPTFILE}" "${SSHUSER}@${CURMACHINE}:${TARGET_TMPFILE}"
      if [ $? -ne 0 ] ; then
          LogError "Error copying the script to the host \"${CURMACHINE}\" - could not execute the script on that host"
          INVALID_HOSTS_LIST="${INVALID_HOSTS_LIST} ${CURMACHINE}"
          continue
      fi
    
      if [ ${ADD_COMMENTS} = ${__TRUE} ] ; then
        echo "# ### ---- Log of the script \"${SCRIPTFILE}\" executed on host \"${CURMACHINE}\" --- start ---" >>"${CUR_OUTPUTFILE}"
      fi
    
      executeCommandAndLog "${SSH_BINARY}" "${SSH_OPTIONS}" -l "${SSHUSER}" "${CURMACHINE}" ${SHELL_TO_USE} ${TARGET_TMPFILE} | tee -a "${CUR_OUTPUTFILE}"
    
      if [ ${ADD_COMMENTS} = ${__TRUE} ] ; then
        echo "# ### ---- Log of the script executed on host \"${CURMACHINE}\" --- end ---" >>"${CUR_OUTPUTFILE}"
        echo "" >>"${OUTPUTFILE}"
      fi
    done
